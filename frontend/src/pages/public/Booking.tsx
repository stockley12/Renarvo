...
import { Check, ChevronRight, CreditCard, Calendar, User, ShieldCheck, PartyPopper, Loader2, AlertTriangle } from 'lucide-react';
...
export default function Booking() {
  ...
  const canCallApi = !!user && user.role === 'customer' && Number.isFinite(numericCarId) && String(numericCarId) === String(car.id);
  ...
  async function confirmBooking() {
    if (!canCallApi) {
      const code = `RNV-${Math.floor(10000 + Math.random() * 90000)}`;
      setConfirmCode(code);
      setStep(3);
      toast.success(t('booking.success'), { description: 'Demo mode: no real reservation created.' });
      return;
    }
    try {
      const payload = {
  ...
  return (
    <div className="container py-6 sm:py-8 max-w-5xl">
      ...
      {!canCallApi && step < 3 && (
        <Card className="mb-4 p-3 bg-warning/5 border-warning/30 text-xs flex items-start gap-2">
          <AlertTriangle className="h-4 w-4 text-warning mt-0.5 shrink-0" />
          <div>
            <strong>Demo booking.</strong>{' '}
            {!user
              ? <>Sign in as a customer to create a real reservation. <Link to="/login" className="text-primary font-semibold underline">Log in</Link></>
              : 'This car is from demo inventory; no real reservation will be created.'}
          </div>
        </Card>
      )}
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-4 lg:gap-6">
        <Card className="p-4 sm:p-6 md:p-8">
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <div className="space-y-4">
                <div><Label>Card number</Label><Input placeholder="4242 4242 4242 4242" inputMode="numeric" /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><Label>Expiry</Label><Input placeholder="MM/YY" inputMode="numeric" /></div>
                  <div><Label>CVC</Label><Input placeholder="123" inputMode="numeric" /></div>
                </div>
                <div><Label>Cardholder name</Label><Input placeholder="AHMET YILMAZ" /></div>
                <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 p-3 rounded-lg">
                  <ShieldCheck className="h-4 w-4 text-success shrink-0" />
                  Pay-on-pickup. No card is charged at this step.
                </div>
              </div>
            </div>
          )}
          {step === 3 && (
            <div className="text-center py-10">
              ...
              <p className="text-sm text-muted-foreground mb-6">{canCallApi ? 'A confirmation email has been sent.' : 'Demo mode — no real booking was created.'}</p>
        ...
        <Card className="p-5 h-fit lg:sticky lg:top-20">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-muted-foreground">{days} day{days > 1 ? 's' : ''} × {formatPrice(car.pricePerDay, currency, locale)}</span><span>{formatPrice(car.pricePerDay * days, currency, locale)}</span></div>
